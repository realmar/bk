...
if [[ $INSTBK =~ ^(yes|y) ]] || [[ -z $INSTBK ]]; then
    ...
    if [[ $INSTPKG =~ ^(yes|y) ]] || [[ -z $INSTPKG ]]; then
        ...
        aptitude install git perl apache2 libmojolicious-perl libinline-perl libinline-c-perl libjson-perl libtemplate-perl libdbi-perl libdbd-sqlite3-perl libio-all-lwp-perl libyaml-tiny-perl libusb-1.0-0 linux-headers-586 libc6-dev libusb-1.0-0-dev sqlite3 make unzip gcc
    else
